...
The fitted life-cycle model incorporates 5 life stages and incorporates stochasticity using a Monte-Carlo simulation approach. The simulation randomly samples variables from known probability distributions to gain inference about an unknown outcome [@gentleMonteCarloSimulation2015].  After the random variables are drawn we use them in deterministic functions to make probabilistic inferences at the life stages of interest [@manlyRandomizationBootstrapMonte2017].  Our model simulates salmon abundance stochasticity by sampling juvenile productivity from assumed stock-recruit relationships, smolt-to-adult return rate models, and age composition values from multinomial distributions and observed data.  Mean and variance parameters for each modeled distribution are estimated from observed data in order to mimic true Lostine River returns.

The life-cycle model includes the following deterministic and stochastic life stages:

1. Natural and hatchery origin adults kept for broodstock or released upstream to spawn naturally are determined by a weir management sliding scale and the number of fish returning to the Lostine River.
1. Successfully spawning adults in the hatchery and natural environment are reduced from those kept for broodstock or released upstream by pre-spawn mortality rates.
1. The number of natural smolt surviving to Lower Granite Dam is determined by a density-dependent Beverton-Holt stock-recruit model, while hatchery smolt is determined by a density-independent relationship.
1. Surviving smolts returning to the Lostine River prior to harvest or weir removal (i.e., tributary escapement) are determined by observed SARs.
1. The age and year of the fishes return comes from a multinomial model.
1. Those fish are then harvested at a rate determined by the harvest sliding scale.
1. The remaining fish escape to the weir and start the process over.
...
The number of adult fish harvested ($h_O$), where $O$, is either natural-origin ($NOR$) or hatchery-origin ($HOR$) is determined by $N^{Trib}_{NOR}$ and the agreed upon sport and tribal harvest exploitation rates **(NEED CITATION)**. 
...
$$
The number of natural-origin brood stock ($BS_{NOR}$) is then calculated using the designated $pNOB$ from the weir management sliding scale (\@ref(tab:plot-sar-mod)),
...
The total number of fish successfully contributing to hatchery production ($S^{Hatchery}$) is then a random variable from a binomial distribution with an assumed prespawn mortality rate in the hatchery environment ($p^{Hatchery}$) and $BS$.

$$
S^{Hatchery} \sim Binom(p^{Hatchery}, BS)
$$

Natural-origin adult fish released upstream of the weir to spawn naturally is

$$
N^{Released}_{NOR} = N^{Weir}_{NOR} - BS_{NOR},
$$
while, hatchery-origin fish released upstream to spawn naturally is controlled by the weir management sliding scale and the set maximum proportion of hatchery-origin spawners ($pHOS_{max}$),

$$
N^{Released}_{HOR} = \frac{(-pHOS_{max} * S_{NOR})}{pHOS_{max} - 1}.
$$
Excess hatchery-origin adults removed ($R_{HOR}$) from the system and not available for future production then becomes,

$$
 R_{HOR} = N^{Weir}_{HOR} - BS_{HOR} - N^{Released}_{HOR}.
$$
The number of fish contributing to natural production ($S$) is a random variable from a binomial distribution with an assumed prespawn mortality rate in nature ($p^{Nature}$) and the total spawners released.

$$
S^{Nature} \sim Binom(p^{Nature}, N^{Released}_{NOR} + N^{Released}_{HOR})
$$
Juvenile productivity in nature follows a Beverton-Holt density dependent function, where $S^{Nature}$ represents the stock and natural-origin juvenile recruits ($J_{NOR}$) are smolt surviving to Lower Granite Dam.

$$
J_{NOR} = \frac{\alpha S}{1 + \beta S}
$$
The parameter $\alpha$ represents the maximum recruitment rate at low spawning abundances and $\beta$ is the density-dependent parameter that governs the functions shape.
