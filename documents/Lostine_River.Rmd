...
The developed model incorporates 5 life stages and stochasticity using a Monte-Carlo simulation approach. The simulation randomly samples variables from known probability distributions to gain inference about an unknown outcome [@gentleMonteCarloSimulation2015]. After the random variables are drawn we use them in deterministic functions to make probabilistic inferences at the life stages of interest [@manlyRandomizationBootstrapMonte2017]. Model design follows a stream type Chinook salmon life cycle in Idaho [@grootPacificSalmonLife1991] and simulates variable abundance by sampling productivity metrics from previously fitted models of natural and hatchery origin stock-recruit relationships, smolt-to-adult return rate, and age compositions using observed data from the Lostine River.
...
The model simulates natural and hatchery abundance across time by estimating progeny returns with either fixed or random prespawn mortality, juvenile productivity, SARs, and age at return values.  Setting these four parameters as fixed, informs the model to return expected values of abundance across time; removing stochasticity.  In contrast, Monte Carlo simulations can be used to introduce stochasticity in abundance by randomly sampling parameters from known probability distributions.

The model begins by determining the maximum proportion of adult hatchery-origin spawners ($pHOS$) allowed upstream of the weir, and the proportion of natural-origin fish allowed within the hatchery brood stock ($pNOB$). The proportions are determined by the estimated number of natural-origin adults returning to the tributary ($N^{Trib}_{NOR}$) and the agreed upon weir management sliding scale (\@ref(tab:plot-sar-mod))**(NEED CITATION)**.

Natural and hatchery origin adults kept for broodstock or released upstream to spawn naturally are determined by a weir management sliding scale and the number of fish returning to the Lostine River.
...
Within the model, we assummed the number of natural-origin ($NOR$) and hatchery-origin ($HOR$) adult fish harvested is determined by $N^{Trib}_{NOR}$ and the agreed upon sport and tribal tributary harvest exploitation rates **(NEED CITATION)**. Table \@ref(tab:harvest-table) shows the exploitation rates used in the model for fish in excess of the closest lower rate. For example, if the natural-origin return was 500 adults, then harvest would equal $(300 * .01) + (500 - 300) * .08 = 19$. 

```{r harvest-table, tab.cap = 'Tributary harvest rate on surplus fish greater than the closest lower rate.'}
tibble("Abundance" = c('<300', '<1000', '<1500', '<2000', '>2000'),
                      "Harvest Rate on Hatchery Excess" = c(.02, .09, .18, .21, .40),
                      "Harvest on Natural Excess" = c(.01, .08, .16, .19, .28)) %>%
  kable() %>%
  kable_styling(bootstrap_options = 'striped')
```


The number of adult fish surviving to the weir ($N^{Weir}$) for each origin {$O$}, and available for brood stock collection and release upstream then becomes,

$$
N^{Weir}_O = N^{Trib}_O - h^{}_O,
$$
where $h_O$ represents the number of adults harvested. The number of natural-origin brood stock ($BS_{NOR}$) is then calculated using the designated $pNOB$ from the weir management sliding scale (\@ref(tab:plot-sar-mod)) and the available fish at the weir.

$$
BS_{NOR} = N^{Weir}_{NOR} * pNOB
$$
Hatchery-origin brood stock ($BS_{HOR}$) becomes the remainder of the total brood stock need ($BS$).

$$
BS_{HOR} = BS - BS_{NOR}
$$

All remaining natural-origin adult fish are released upstream ($N^{Released}_{NOR}$) of the weir and are available to spawn naturally,

$$
N^{Released}_{NOR} = N^{Weir}_{NOR} - BS_{NOR},
$$
while, hatchery-origin fish released upstream ($N^{Released}_{HOR}$) to spawn naturally is controlled by the weir management sliding scale and set to the maximum proportion of hatchery-origin spawners ($pHOS_{max}$).

$$
N^{Released}_{HOR} = \frac{(-pHOS_{max} * S_{NOR})}{pHOS_{max} - 1}.
$$

Excess hatchery-origin adults that were not collected for brood stock or released upstream are considered removed from the system and not available to produce offspring.

Once fish are designated into the correct spawning environment, natural or hatchery, The total number of fish successfully contributing to juvenile production ($S_{O}$) is then a random variable from a binomial distribution with an assumed prespawn mortality rate ($p_O$) and the number of available spawners.

Juvenile productivity in the natural environment is assumed to follow a Beverton-Holt density dependent function, where $S^{NOR}$ represents the stock and natural-origin juvenile recruits ($J_{NOR}$) are smolt surviving to Lower Granite Dam.
...
We assume hatchery-origin smolt $J^{HOR}$ surviving to Lower Granite Dam is a linear function of the total brood stock spawned. Stochasticity is introduced by sampling the the ratio of hatchery smolts to hatchery spawners ($\phi$) from a log-normal distribution,

$$
\phi \sim LogNormal(\mu_\phi, \sigma_\phi)
$$

where, $\mu_\phi$ and $\sigma_\phi$ is estimated from observed data. Hatchery-origin juvenile smolts surviving to Lower Granite Dam then becomes,

$$
J_{HOR} = S^{Hatchery} * \phi.
$$
...
Brood year returns of NOR and HOR fish .
...
Total progeny return ($P_{i,t}$), regardless of origin ($i$), were assumed to return following a multinomial distribution;

\begin{equation}
	[P_{i,3,t},P_{i,4,t},P_{i,5,t}]^T \sim Multinomial(P_{i,t},\pi_i).
\end{equation} 

With $\pi_i$ being a mean vector of age proportions supplied by the user for origin $i$, and $P_{i,j,t}$ being the number of progeny that returned from year $t$ at age $j= 3, 4$ or $5$.
...
The Monte-Carlo model was written in the statistical computing language R (R Core Team 2014) and available in an R package at [https://github.com/NPTfisheries/Chinook-Life-Cycle](https://github.com/NPTfisheries/Chinook-Life-Cycle).
